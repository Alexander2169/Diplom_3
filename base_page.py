...
class BasePage:
    """Базовый класс для работы со страницами в браузере."""
    ...
    def get_visible_element(self, locator):
        ...
        try:
            ...
            self.print_error(f'Элемент {locator} не найден в окне браузера')
    ...
    def is_element_invisible(self, locator):
        """Проверяет, что элемент невидим на странице."""
        try:
    ...
    def wait_for_text_change(self, locator, old_text):
        """Ожидает изменения текста элемента на странице."""
        try:
            WebDriverWait(self.driver, 5).until_not(
                expected_conditions.text_to_be_present_in_element(locator, old_text))
            return True
        except Exception as e:
            self.print_error(f'Проблема: {e}')
            return False

    def get_clickable_element(self, locator):
        """Находит кликабельный элемент на странице по локатору."""
        try:
            return WebDriverWait(self.driver, 5).until(
                expected_conditions.element_to_be_clickable(locator))
        except (TimeoutException, NoSuchElementException):
            self.print_error(f'Элемент {locator} не найден/не кликабелен')

    def click_element(self, locator):
        """Кликает на элемент, если он кликабелен."""
        try:
    ...
    def drag_and_drop(self, source_locator, target_locator):
        """Перетаскивает элемент из одного места в другое."""
    ...
    def verify_page_opened(self, expected_url, visible_element_locator):
        """Проверяет, что страница открыта и отображается ожидаемый элемент."""
